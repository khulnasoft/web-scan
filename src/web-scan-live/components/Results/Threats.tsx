    ...
    <Card heading={props.title} actionButtons={props.actionButtons}>
      {safeBrowsing && !safeBrowsing.error && (
        <Row
          ...
          val={safeBrowsing.unsafe ? "❌ Unsafe" : "✅ Safe"}
        />
      )}
      {((cloudmersive && !cloudmersive.error) || safeBrowsing?.details) && (
        <Row
          lbl="Threat Type"
          val={
            safeBrowsing?.details?.threatType ||
            cloudmersive.WebsiteThreatType ||
            "None :)"
          }
        />
      )}
      {phishTank && !phishTank.error && (
        <Row
          lbl="Phishing Status"
          val={
            phishTank?.url0?.in_database !== "false"
              ? "❌ Phishing Identified"
              : "✅ No Phishing Found"
          }
        />
      ...
      {phishTank.url0 && phishTank.url0.phish_detail_page && (
        <Row lbl="" val="">
          ...
          <span className="val">
            <a href={phishTank.url0.phish_detail_page}>
      ...
      {urlHaus.urls && (
        <Expandable>
          ...
          {urlHaus.urls.map((urlResult: any, index: number) => {
            ...
            return (
              <ExpandableRow
                lbl={getExpandableTitle(urlResult)}
                val=""
                rowList={rows}
              />
            );
