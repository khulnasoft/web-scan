...
import { ExpandableRow } from "web-scan-live/components/Form/Row";

const makeCipherSuites = (results: any) => {
  if (
    !results ||
    !results.connection_info ||
    (results.connection_info.ciphersuite || [])?.length === 0
  ) {
    return [];
  }
  return results.connection_info.ciphersuite.map((ciphersuite: any) => {
    return {
      ...
      fields: [
        ...
        {
          lbl: "OCSP Stapling",
          val: ciphersuite.ocsp_stapling ? "✅ Enabled" : "❌ Disabled",
        },
        { lbl: "PFS", val: ciphersuite.pfs },
        ciphersuite.curves
          ? { lbl: "Curves", val: ciphersuite.curves.join(", ") }
          : {},
      ],
    };
  });
...
const TlsCard = (props: {
  ...
  useEffect(() => {
    // Update cipher suites when data changes
  ...
  return (
    <Card heading={props.title} actionButtons={props.actionButtons}>
      {cipherSuites.length &&
        cipherSuites.map((cipherSuite: any, index: number) => {
          return (
            <ExpandableRow
              key={`tls-cipher-${index}`}
