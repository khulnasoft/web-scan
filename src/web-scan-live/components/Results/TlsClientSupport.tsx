...
const makeClientSupport = (results: any) => {
  ...
  const sslLabsClientSupport = results.analysis.find(
    (a: any) => a.analyzer === "sslLabsClientSupport"
  ).result;

  return sslLabsClientSupport.map((sup: any) => {
    return {
      title: `${sup.name} ${
        sup.platform ? `(on ${sup.platform})` : sup.version
      }`,
        ...
        : [
            {
              ...
              val: "",
              plaintext:
                `The host ${target} does not support ${sup.name} ` +
