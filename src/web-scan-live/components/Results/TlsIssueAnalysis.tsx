...
import Row, { ExpandableRow } from "web-scan-live/components/Form/Row";
  ...
  return results.analysis.map((analysis: any) => {
    const fields = Object.keys(analysis.result).map((label) => {
      ...
      return {
        lbl,
        val: "",
        plaintext: JSON.stringify(analysis.result[label]),
      };
    });
    return {
...
const makeResults = (results: any) => {
  ...
  const caaWorker = results.analysis.find(
    (a: any) => a.analyzer === "caaWorker"
  );
  if (caaWorker.result.host)
    rows.push({ lbl: "Host", val: caaWorker.result.host });
  if (typeof caaWorker.result.has_caa === "boolean")
    rows.push({ lbl: "CA Authorization", val: caaWorker.result.has_caa });
  if (caaWorker.result.issue)
    rows.push({
      lbl: "CAAs allowed to Issue Certs",
      plaintext: caaWorker.result.issue.join("\n"),
    });

  const mozillaGradingWorker = results.analysis.find(
    (a: any) => a.analyzer === "mozillaGradingWorker"
  ).result;
  if (mozillaGradingWorker.grade)
    rows.push({ lbl: "Mozilla Grading", val: mozillaGradingWorker.grade });
  if (mozillaGradingWorker.gradeTrust)
    rows.push({ lbl: "Mozilla Trust", val: mozillaGradingWorker.gradeTrust });

  const symantecDistrust = results.analysis.find(
    (a: any) => a.analyzer === "symantecDistrust"
  ).result;
  if (typeof symantecDistrust.isDistrusted === "boolean")
    rows.push({
      lbl: "No distrusted symantec SSL?",
      val: !symantecDistrust.isDistrusted,
    });
  if (symantecDistrust.reasons)
    rows.push({
      lbl: "Symantec Distrust",
      plaintext: symantecDistrust.reasons.join("\n"),
    });

  const top1m = results.analysis.find(
    (a: any) => a.analyzer === "top1m"
  ).result;
  if (top1m.certificate.rank)
    rows.push({
      lbl: "Certificate Rank",
      val: top1m.certificate.rank.toLocaleString(),
    });

  const mozillaEvaluationWorker = results.analysis.find(
    (a: any) => a.analyzer === "mozillaEvaluationWorker"
  ).result;
  if (mozillaEvaluationWorker.level)
    rows.push({
      lbl: "Mozilla Evaluation Level",
      val: mozillaEvaluationWorker.level,
    });
  if (mozillaEvaluationWorker.failures) {
    const { bad, old, intermediate, modern } = mozillaEvaluationWorker.failures;
    if (bad)
      rows.push({ lbl: `Critical Security Issues (${bad.length})`, list: bad });
    if (old)
      rows.push({
        lbl: `Compatibility Config Issues (${old.length})`,
        list: old,
      });
    if (intermediate)
      rows.push({
        lbl: `Intermediate Issues (${intermediate.length})`,
        list: intermediate,
      });
    if (modern)
...
const TlsCard = (props: {
  ...
  return (
    <Card heading={props.title} actionButtons={props.actionButtons}>
      {tlsResults.length > 0 &&
        tlsResults.map((row: any, index: number) => {
          return (
            <Row
              lbl={row.lbl}
              val={row.val}
      ...
      {!tlsRowData.length && (
        <div>
          <p>
            ...
            <br />
            This sometimes happens when the report didn't finish generating
            in-time, you can try re-requesting it.
          </p>
