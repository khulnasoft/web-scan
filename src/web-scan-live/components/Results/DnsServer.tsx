      ...
      {dnsSecurity.dns.map((dns: any, index: number) => {
        return (
          <div key={`dns-${index}`}>
            ...
            <Row
              lbl="DoH Support"
              val={dns.dohDirectSupports ? "✅ Yes*" : "❌ No*"}
              key={`doh-${index}`}
            />
          </div>
        );
      })}
      {dnsSecurity.dns.length > 0 && (
        <small>
          * DoH Support is determined by the DNS server's response to a DoH
          ...
          that the DNS server supports DoH but does not respond to DoH queries.
          If the DNS server does not support DoH, it may still be possible to
          use DoH by using a DoH proxy.
        </small>
      )}
    </Card>
  );
